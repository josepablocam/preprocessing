...
class QueryDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.num_obs = self.queries.shape[0]

    def __len__(self):
        return self.num_obs

    def __getitem__(self, ix):
        return self.queries[ix]
...
class CrossDomainDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        # docstrings -> original
        # queries -> target
        self.original_domain_obs = original_domain_obs
        self.target_domain_obs = target_domain_obs
        self.n_original = len(self.original_domain_obs)
        self.n_target = len(self.target_domain_obs)
        self._len = max(self.n_original, self.n_target)

    def __len__(self):
        return self._len

    def __getitem__(self, ix):
        # ignore the ix fed in...
        randix_original = np.random.randint(0, self.n_original)
        randix_target = np.random.randint(0, self.n_target)
        return (
            self.original_domain_obs[randix_original],
            self.target_domain_obs[randix_target],
        )
